    ...
    async def on_message(self, message):
        ...
        if user.id in self.accepted_list:
            ...
            if split_message[0] == "help":
                ...
                new_embed.add_field(name="Request Averages",value="request {username/op.gg (replace spaces with underscores)} avg {number of games} {optional-Name of champion (replace spaces with underscores)}",inline=False)
                new_embed.add_field(name="Request Ranked Averages",value="request {username/op.gg (replace spaces with underscores)} rankedavg {number of games} {optional-Name of champion (replace spaces with underscores)}",inline=False)
                new_embed.add_field(name="Request Games",value="request {username/op.gg (replace spaces with underscores)} games {number of games} {optional-Name of champion (replace spaces with underscores)}",inline=False)
                new_embed.add_field(name="Request Custom Game/Match",value="matchrequest {matchid} {Name of champion played (replace spaces with underscores)}",inline=False)
                new_embed.add_field(name="Request Item",value="item {Name of item (replace spaces with underscores)/list - to see all items }",inline=False)
                new_embed.add_field(name="Change Region",value="cr {possible regions are: BR1,EUN1,EUW1,JP1,KR,LA1,LA2,NA1,OC1,TR1,RU}  example usage: cr NA1",inline=False)
                new_embed.add_field(name="Multi Request",value="multirequest {op.gg multi}",inline=False)
                new_embed.add_field(name="Check Region",value="region",inline=False)
